...
For overload methods it will use the first signature if the signatures are indistinguishable or if the overloads feature is disabled.
...
For overload constructors it will use the first one, or if the overloads feature is enabled, use the matching signature, i.e.:
...
const mock = createMock<Person>();
new mock() // { shirtSize: 0 }
new mock(0) // { hatSize: 0 }
mock.name // ""
```
